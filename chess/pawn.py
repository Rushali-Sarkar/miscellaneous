...
SPACE = " "
WHITE, BLACK = "P", "p"
can_move_from = {WHITE: {"3": ["2"], "4": ["2", "3"], "5": ["4"], "6": ["5"], "7": ["6"]}, BLACK: {"6" : "7" , "5": "6", "4": "5", "3": "4", "2": "3", "1": "2"}}

can_capture_from = {WHITE: {"3": "2", "4": "3", "5": "4", "6": "5", "7": "6", "8": "7"}, BLACK: {"6": "7", "5": "6", "4": "5", "3": "4", "2": "3", "1": "2"}}
...
def is_promotion(move):
    return re.match("[Pp][a-h]([18]|x[a-h][18])[RNBQrnbq]", move) is not None
...
def capture(move, board_view, piece_view):
    ...
    #piece_View_update tbd
    return board_view, piece_view

def promote(move, board_view, piece_view):
    if "x" in move:
        # Move like gxf8 = Q or gfx8Q
...
def make_enpassant(move, board_view, piece_view):
    ...
    captured_pawn_square = to_file + enpassant_capture[pawn]
    ...
    piece_view[pawn].remove(from_square)
    piece_view[pawn].remove(to_square)
